...
function CartPage() {
    ...
    function handleApplyCoupon() {
        setActiveCode(couponRef.current.value); // Update the active coupon code
    }
        ...
        <div className='flex flex-col items-center justify-between'>
            ...
            <div className="container mx-auto p-2 flex justify-between flex-col lg:flex-row gap-4">
                ...
                <div className='w-full lg:w-[30%] border border-gray-300 rounded-lg gap-4 flex flex-col'>
                    <div className='w-full h-[40px] bg-primaryColor flex justify-between items-center p-6 rounded-lg'>
                        <div className='text-white flex items-center justify-center'>Cart Total</div>
                    </div>
                    <div className='p-4'>
                        {cart.length > 0 && (
                            <div>
                                <div className="flex flex-col justify-between gap-4 ">
                                    ...
                                    <p className="font-bold py-4">Total Price: ${activeCode === 'Djordje' ? (totalPrice * 0.90).toFixed(2) : totalPrice.toFixed(2)}</p>
                                </div>
                                <div className='flex justify-between flex-row bg-gray-200'>
                                    <input 
                                        ref={couponRef}
                                        type="text" className='w-full' placeholder='Enter coupon code' />
                                    <button
                                        onClick={handleApplyCoupon}
                                        className='bg-secondaryColor rounded-lg px-4 py-2'>Apply</button>
                                </div>
